  ...
  files.forEach((file) => {
    ...
    fs.createReadStream(file.path)
      ...
      .on("data", function (row) {
        userName = row.shift();
        domains = row.shift().replace(/\s+/g, "").trim().split("|");

        keyWords = [...row].map((keyWord) =>
          keyWord.replace(/\s+/g, "").trim().split("|").join(",")
        );

        if (domains.length != keyWords.length) {
          response.send({
            error:
              "domains and keywors number not matched or there is empty filed",
          });
        }

        domains.forEach((domain, index) => {
          saveToDomainKeyWord(domain, keyWords[index], userName).catch((err) =>
            response.send(err)
          );
        });
      })
